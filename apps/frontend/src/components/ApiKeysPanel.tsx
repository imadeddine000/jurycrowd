  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex items-center gap-2 border-b border-hairline bg-surface-soft/40 px-3 py-2">
        ...
        <Key className="h-4 w-4 text-muted-ink" />
        <span className="text-title-sm text-ink">API Keys</span>
      </div>
      <div className="flex-1 overflow-y-auto p-4">
        <div className="mx-auto max-w-2xl space-y-4">
          <Card variant="outline">
            <CardHeader><CardTitle>{editing ? 'Edit API Key' : 'Add API Key'}</CardTitle></CardHeader>
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 text-center">
              <Key className="h-10 w-10 text-muted-ink/30" />
              <p className="mt-3 text-body-sm text-muted-ink">No API keys yet. Add one above.</p>
              <p className="mt-1 text-caption text-muted-soft-ink">Keys are injected as env vars when launching agents.</p>
            </div>
          ) : (
            <div className="space-y-2">
              {keys.map((k) => (
                <Card variant="outline" key={k.id}>
                  <CardContent className="flex items-center gap-3 py-3">
                    <div className="flex-1 space-y-0.5">
                      <div className="flex items-center gap-2">
                        <span className="text-body-sm font-medium text-ink">{k.name}</span>
                        <code className="rounded-sm bg-surface-soft px-1.5 py-0.5 font-mono text-caption text-muted-ink">{k.envVar}</code>
                      </div>
                      <div className="flex items-center gap-3 text-caption text-muted-ink">
                        <span>{k.key}</span>

      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="w-56 shrink-0 border-r p-2">
          ...
          <div className="mt-2 overflow-y-auto" style={{ maxHeight: 'calc(100% - 40px)' }}>
            {files.length === 0 ? (
              ...
              <div key={name} draggable onDragStart={(e) => { e.dataTransfer.setData('text/plain', `.agent-workspace/${kind}/${name}.md`); }} className={cn('group flex items-center gap-1 rounded px-2 py-1.5 text-sm cursor-pointer', selected === name ? 'bg-accent' : 'hover:bg-secondary')} onClick={() => loadFile(name)}>
        ...
        <div className="flex-1 overflow-hidden" data-color-mode={isDark ? 'dark' : 'light'}>
          {loading ? (
            ...
            <div className="flex h-full flex-col">
              <span className="border-b px-3 py-1 text-xs text-muted-foreground">{selected}.md</span>

...
// POST /api/sessions/:id/send-text — send text to a tmux session (drag-and-drop)
sessionsRouter.post('/sessions/:id/send-text', async (req, res) => {
  try {
    const { text } = req.body as { text: string };
    const session = await prisma.agentSession.findUnique({ where: { id: req.params.id } });
    if (!session) return res.status(404).json({ error: 'Session not found' });
    tmux.sendKeys(session.tmuxSession, text);
